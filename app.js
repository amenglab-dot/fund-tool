...
const PositionReasons = {
    getReason(fund, deviation, dayChange, weekChange, monthReturn) {
        ...
        } else if (deviation > 0.3 && deviation <= 0.8) {
            reasons.push('估值略高，可适当减仓锁定收益');
        } else if (deviation < -0.3 && deviation >= -0.8) {
            reasons.push('估值偏低，是加仓布局好时机');
        }
        ...
        if (dayChange > 2.5) {
            ...
            reasons.push('大幅回调，可逢低分批加仓');
        }

        // 基于周涨跌
        if (weekChange > 6) {
            reasons.push('周涨幅较高，建议减仓20%');
        } else if (weekChange < -6) {
            reasons.push('周跌幅较大，是加仓好时机');
        } else if (weekChange > 3) {
            reasons.push('周收益可观，可部分止盈');
        }

        // 基于月收益
        if (monthReturn > 8) {
            reasons.push('月涨幅较大，注意风险控制');
    ...
    getSuggestion(deviation, dayChange, weekChange, monthReturn, fund) {
        let score = 0;
...
const Charts = {
    ...
    initProfitChart(period = '1M') {
        ...
        for (let i = days; i >= 0; i--) {
            ...
            const fundDailyReturn = (Math.random() - 0.42) * 2.5;
            const benchDailyReturn = (Math.random() - 0.45) * 2;

            fundValue *= (1 + fundDailyReturn / 100);
            benchmarkValue *= (1 + benchDailyReturn / 100);
                    ...
                    tooltip: {
                        ...
                        callbacks: {
                            label: function(context) {
                                ...
                                return context.dataset.label + ': ¥' + parseFloat(value).toLocaleString();
                            }
                        }
                    }
                },
...
const NewsData = {
    news: [
        { id: 1, title: '央行降准0.25个百分点，释放长期资金约5000亿元，市场流动性充足', source: '央视财经', time: '10分钟前', category: 'policy', hot: true },
        { id: 2, title: 'A股成交额突破1.5万亿元创年内新高，资金跑步入场', source: '证券时报', time: '30分钟前', category: 'market', hot: true },
        { id: 3, title: '公募基金规模突破31万亿元权益类基金表现亮眼', source: '中国基金报', time: '1小时前', category: 'fund', hot: true },
        { id: 4, title: '新能源汽车销量同比增长35%，产业链景气度持续上行', source: '财新', time: '2小时前', category: 'market', hot: false },
        { id: 5, title: '证监会发布公募基金费率改革方案，降低投资者成本', source: '证监会发布', time: '3小时前', category: 'policy', hot: true },
        { id: 6, title: '消费板块估值处于历史低位，机构看好后市中长期表现', source: '券商研报', time: '4小时前', category: 'market', hot: false },
        { id: 7, title: 'QDII基金年内表现突出，港股科技板块获资金青睐', source: '天天基金', time: '5小时前', category: 'fund', hot: false },
        { id: 8, title: '科创50指数持续走强，科技成长风格占优', source: 'Wind', time: '6小时前', category: 'market', hot: true }
    ],
...
const UI = {
    ...
    renderValuationTable() {
        ...
        const fundTypeMap = { stock: '股票型', bond: '债券型', mix: '混合型', index: '指数型' };
        ...
        tbody.innerHTML = funds.map((fund, index) => {
            ...
            const suggestionData = PositionReasons.getSuggestion(deviation, fund.dayChange, weekChange, monthReturn, fund);
            ...
            return `
                <tr class="fade-in" style="animation-delay: ${index * 0.05}s">
                    ...
                    <td class="valuation-cell font-medium">${fund.netValue.toFixed(4)}</td>
                    ...
                    <td>
                        <span class="deviation ${estimateChange >= 0 ? 'up' : 'down'}">
                            ${estimateChange >= 0 ? '↑' : '↓'}${Math.abs(estimateChange).toFixed(2)}%
                        </span>
                    </td>
                    <td class="${fund.dayChange >= 0 ? 'positive-text' : 'negative-text'}">
                        ${fund.dayChange >= 0 ? '+' : ''}${fund.dayChange.toFixed(2)}%
                    </td>
                    <td class="${weekChange >= 0 ? 'positive-text' : 'negative-text'}">
                        ${weekChange >= 0 ? '+' : ''}${weekChange.toFixed(2)}%
                    </td>
                    <td class="${monthReturn >= 0 ? 'positive-text' : 'negative-text'}">
                    ...
                    <td style="min-width: 160px; text-align: left; padding: 8px;">
                        <span class="suggestion ${suggestionData.className}" style="display: inline-block; margin-bottom: 4px; font-size: 12px;">
                            ${suggestionData.action}
                        </span>
                        <div class="suggestion-reason" title="${reasonsData.fullReasons}" style="font-size: 11px; color: var(--text-tertiary); cursor: help;">
                            ${reasonsData.reasons[0] || '持有观望'}
                        </div>
                        <div class="action-hint" style="font-size: 10px; color: var(--primary-color); margin-top: 2px;">
                            ${suggestionData.actionDesc}
                        </div>
                    </td>
                    <td>
                        <div class="action-btns">
                            <button class="action-btn" title="查看详情" onclick="UI.showFundDetail('${fund.code}')">
                                <i class="fas fa-chart-bar"></i>
                            </button>
                            <button class="action-btn delete" title="移出自选" onclick="UI.removeFund('${fund.code}')">
    ...
    renderPortfolio() {
        ...
        if (holdingsList && portfolio.funds.length > 0) {
            ...
            holdingsList.innerHTML = portfolio.funds.map((fund, i) => {
                const ratio = (fund.value ? fund.value / totalValue : 1 / portfolio.funds.length * 100);
                const fundValue = fund.value ? fund.value : fund.netValue * fund.holdings;
                ...
                return `
                    <div class="holding-item fade-in" style="animation-delay: ${i * 0.05}s">
                        ...
                        <div class="holding-info">
                            ...
                            <div class="holding-code">${fund.code} · ${(ratio * 100).toFixed(1)}%</div>
                        </div>
                        <div class="holding-value" style="text-align: right;">
                            <div style="font-weight: 600; color: var(--text-primary);">¥${parseInt(fundValue).toLocaleString()}</div>
                            <div style="font-size: 11px; color: var(--text-tertiary);">持仓${fund.holdings.toFixed(1)}%</div>
                        </div>
                    </div>
                `;
            }).join('');
    ...
    renderRecommend() {
        ...
        const sorted = [...FundData.allFunds]
            .sort((a, b) => b.yearReturn - a.yearReturn)
            .slice(0, 5);
        const typeMap = { stock: '股票型', bond: '债券型', mix: '混合型', index: '指数型' };
        ...
        results.innerHTML = funds.map(fund => {
            ...
            return `
                <div class="search-result-item">
                    ...
                    <button class="add-btn-mini" onclick="UI.addToWatchlist('${fund.code}')">
    ...
    addToWatchlist(code) {
        ...
        const funds = [...FundData.allFunds].filter(f =>
            f.code.includes(code) || f.name.includes(code)
        );
        this.renderSearchResults(funds);
    ...
    init() {
        ...
        document.querySelectorAll('.filter-chip').forEach(chip => {
            chip.addEventListener('click', function() {
                ...
                UI.showToast(`📊 筛选: ${this.textContent}`);
            });
        });
        ...
        addFundBtn?.addEventListener('click', () => {
            ...
            UI.renderSearchResults(FundData.allFunds.slice(0, 5));
        });
        ...
        const handleModalSearch = () => {
            const keyword = modalSearch.value.trim();
            const results = FundData.allFunds.filter(f =>
        ...
        refreshRecommend?.addEventListener('click', () => {
            UI.renderRecommend();
            UI.showToast('🔄 已换一换');
        });
        ...
        exportBtn?.addEventListener('click', () => {
            UI.showToast('📥 正在导出...');
            setTimeout(() => UI.showToast('✅ 导出成功'), 1000);
        });
    }
};
